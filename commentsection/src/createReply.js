...
if (window < 769) {
  return (
    ...
    <div className="replyBox mb-5">
        <textarea type="text" maxLength="256" name="Message" className="replyContentBox" value={ReplyMessage} onChange={handleChange}/>
        <img className="replyProfileimg" src={props.currentUser.image.png}/>
        <button className="bigReplyButton" onClick={() => props.Save(ReplyMessage)}>SEND</button>
     </div>
    </div>
  )
}
